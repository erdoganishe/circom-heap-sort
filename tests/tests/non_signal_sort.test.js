...
describe("NonSignalSort test", function () {
    this.timeout(1000000);


	it("NonSignalSort 1 (7_4) test", async function () {
		...
		try {
			...
			const circuit = await wasm_tester(
				path.join(__dirname, "./circuits/non_signal_sort/7_4.circom")
			);
    ...
    it("NonSignalSort 2 (12_4) test", async function () {
		...
		try {
			...
			const circuit = await wasm_tester(
				path.join(__dirname, "./circuits/non_signal_sort/12_4.circom")
			);
			const w = await circuit.calculateWitness({ 
               in: input.in
            });
	...
	it("NonSignalSort 3 (256_10) test", async function () {
		...
		try {
			...
			const circuit = await wasm_tester(
				path.join(__dirname, "./circuits/non_signal_sort/256_10.circom")
			);
			const w = await circuit.calculateWitness({ 
               in: input.in
            });
